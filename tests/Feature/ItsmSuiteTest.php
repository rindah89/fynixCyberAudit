...
class ItsmSuiteTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        foreach (['company_id', 'origin_id', 'ticket_type_id', 'department_id', 'priority_id', 'sync_analyst_id'] as $key) Config::set('suite.itsm.'.$key, 1);
    ...
    public function test_signed_unknown_itsm_event_is_safely_ignored(): void
    {
        ...
        $body = json_encode(['event_type' => $event, 'tenant_id' => (string) Str::uuid(), 'entity_type' => 'course', 'entity_id' => (string) Str::uuid(), 'occurred_at' => now()->utc()->format('Y-m-d\TH:i:s\Z'), 'payload' => []], JSON_UNESCAPED_SLASHES);
        $timestamp = time();
