...
After independent terminal closure, a case manager submits `POST /api/compliance-cases/{case}/closure-reports` with a required bounded `executive_summary`. The server locks the closed case and governed source histories, requires the latest independently approved investigation report, retains the complete current case/event and approved-report/review evidence plus an ordered fingerprint registry for intake/disposition, evidence, interview, hold, investigation, and action-lifecycle sources, renders a private PDF, and records exact size/SHA-256 plus a recursively canonicalized evidence fingerprint. At most 20 versions are retained. `GET /api/compliance-cases/{case}/closure-reports` returns paginated safe metadata to current case readers and omits the internal snapshot/storage path. `GET /app/compliance-case-closure-reports/{report}/download` independently rechecks current case access and streams only after bounded retained-size/SHA-256 verification; missing bytes return 404 and mismatch returns 409. This is a deliberate retained closure package, not a legal opinion, regulatory filing/delivery receipt, source-authenticity/sufficiency determination, or assurance of investigative quality/effectiveness.
