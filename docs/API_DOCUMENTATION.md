...
Authenticated users list only their assignments, initial delivery, reminder, and escalation evidence through `GET /api/policy-acknowledgements/mine?page=1&per_page=50` and acknowledge their own open assignment through `POST /api/policy-acknowledgement-assignments/{assignment}/acknowledge` with `{"acknowledged": true}` plus optional `comment` and `client_reference`. The server owns the acknowledgement statement, actor, time, and snapshots. Policy owners/editors use `GET /api/policy-acknowledgement-campaigns/{campaign}/report` for paginated assignment/delivery/reminder/escalation status and `POST /api/policy-acknowledgement-campaigns/{campaign}/close` for one governed closure. List page size is capped at 100. Acknowledgements remain accepted after the due time until closure; closure prevents later submissions. Evidence proves insertion into Fynix's database notification store, not human viewing/readership or email/SMS/push receipt. Policy-owner escalation does not prove or infer an HR/line-management hierarchy. This is attributable application acknowledgement, not qualified electronic signature, identity proofing, HR audience synchronization, training completion, or compliance inference.
