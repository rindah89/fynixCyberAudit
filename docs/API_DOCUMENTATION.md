...
The exact active delivered recipient may use the provider workspace to acknowledge closure once. The immutable `acknowledgement` binds the material recipient identity/account state and complete closure/delivery snapshots with time and fingerprint. Staff history receives the full ACL-filtered evidence; the provider projection contains only acknowledgement time and fingerprint. There is deliberately no public or staff mutation endpoint. This action is not evidence of comprehension, agreement, contractual assent/discharge, response truth, fulfillment, or performance.
