...
During `Investigating` or `Action Required`, the assigned investigator or a case manager records one immutable conclusion per approved-plan procedure through `POST /api/compliance-cases/{case}/investigation-procedure-executions`. `procedure_index` (1–50), `result` (`completed`, `exception_identified`, or `not_applicable`), and `summary` are required; `findings` is required for `exception_identified`, and `source_reference` is optional. The server derives the exact procedure text and retains complete approved plan/review plus current case/event snapshots, executor/time, and fingerprint. `GET` on the same route exposes paginated complete evidence to current case readers. A newly plan-governed case cannot resolve until every approved-plan procedure has one retained conclusion. These deliberate records do not prove execution, source authenticity, truth, legal sufficiency, investigation quality, or effectiveness.
