...
Governed work-program endpoints are `GET /api/audits/{audit}/procedures`, `POST /api/audits/{audit}/procedures`, and `POST /api/audit-procedures/{procedure}/execute`. Definition requires `audit_item_id`, `code`, `title`, `objective`, `steps`, `method` (`inquiry`, `inspection`, `observation`, `reperformance`, or `analytics`), and `assigned_to`; optional fields are `population_description`, `planned_sample_size` (1–1,000,000), and an audit-period `due_at` in `Y-m-d`. Execution requires `outcome` (`effective`, `needs_improvement`, `ineffective`, or `not_applicable`) and `result`; optional fields are `exceptions`, `sample_tested` (0–1,000,000 and not above the plan), and unverified `evidence_reference`. Server-owned audit/version/status/actor/time/snapshot/fingerprint fields are prohibited. Work programs are bounded to 250 procedure versions and histories accept `page` and `per_page` (1–100). Procedures and results remain deliberate inputs; Fynix does not generate procedures, select samples, inspect evidence content, or infer outcomes.
