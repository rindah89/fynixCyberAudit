...
- Before migrations and at least hourly, create a consistent MySQL dump plus copies of persistent uploaded evidence and the production `.env`/application key. Encrypt backups off-host; retain at least 7 daily, 4 weekly, and 12 monthly recovery points unless policy is stricter.
...
- Configure `FYNIX_BACKUP_AGE_RECIPIENT`, `FYNIX_BACKUP_AGE_IDENTITY_FILE`, `FYNIX_SECRET_BACKUP_REF`, and protected off-host `FYNIX_BACKUP_S3_URI`, then run `sudo deploy/install-recovery-schedule.sh`. The installed timers create an encrypted backup hourly and execute a fresh isolated restore drill on the first day of January, April, July, and October. Monitor failed units; a failed or unreviewed drill keeps DG-09 partial.
