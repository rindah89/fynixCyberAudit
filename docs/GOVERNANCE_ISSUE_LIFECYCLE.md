...
Fynix provides one governed lifecycle for issues opened by risk portfolio reviews, third-party risk reviews, AI monitoring reviews, operational-resilience exercises, and continuous control tests. The lifecycle connects these source records to the existing remediation workspace without claiming automatic remediation execution or verified evidence ingestion.
...
3. `verification` → `closed`: a user with `Verify Issue Closure` records a verification summary. The verifier must be different from the issue owner and the remediation task owner and assignee, and the task must remain complete under the closure lock.
...
The **Remediation → Governance Issues** workspace provides owner-scoped discovery, current status, remediation linkage, closure details, actions allowed by permission and state, append-only transition history, and private-disk export. The source issue remains visible in its existing domain workspace.
...
Closure stores an attributable verification summary and may store an operator-supplied external `evidence_reference`. That reference is not resolved, authorized, hashed, retained, or verified by this workflow. The task snapshot proves the task values considered at each transition, not that the underlying corrective action or evidence is independently authentic.

Fynix does not automatically select treatments, execute corrective actions, ingest or validate closure evidence, infer effectiveness from telemetry, waive issues, or close issues when a task changes. Remediation task completion and lifecycle transitions remain deliberate operator actions. Verified evidence linkage and automatic remediation remain separate gaps in the IRM evidence ledger.
