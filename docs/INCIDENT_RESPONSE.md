...
11. An incident manager may append up to 100 affected assets, applications, vendors, controls, or risks. Each immutable record captures the selected entity's material current values, a required impact summary, actor/time, and SHA-256 fingerprint; affected controls also require a failure note. Later source-record changes or deletion do not rewrite the incident-scope evidence.
12. An incident manager may append up to 500 timestamped observations, actions, decisions, or communications to the governed timeline. Every entry retains its immutable incident snapshot, actor/time, version, summary/details, pin state, visibility, and SHA-256 fingerprint. `Internal` entries are restricted to current incident editors; `Auditor Visible` entries are available to authorized incident readers and are the only manual timeline records eligible for a final report.
13. After the incident reaches `Lessons Learned`, a manager with incident-evidence permission may generate up to 20 point-in-time final-report versions. The server locks and snapshots the incident, phase decisions, current tasks/latest event fingerprints, all governed phase/task evidence up to 5,000 manifest rows, notification status, affected entities, auditor-visible timeline only, and lessons. It renders and retains a bounded PDF with exact size/SHA-256 plus a fingerprint over the complete report record. Download rechecks incident view, `Manage Incident Evidence`, and every included source-attachment ACL, then verifies the retained PDF bytes before returning them.
...
- Affected-entity links are append-only and unique per incident/type/source ID. The service locks the incident then selected source, repeats current incident-update authorization, and owns the immutable source snapshot, actor/time, and fingerprint.
- Timeline entries are append-only and server-versioned under the incident lock. Caller-owned versions, snapshots, attribution, recorded times, and fingerprints are rejected. Read-only incident users never receive internal entries through REST or the operator workspace.
- Final reports are immutable point-in-time records, not mutable incident state. Generation is limited to `Lessons Learned`, 20 versions, 5,000 evidence-manifest rows, a 5,000,000-byte serialized snapshot, and a 10 MiB PDF. REST exposes report metadata but not the protected snapshot/path; operator inspection and exact PDF download require incident-evidence permission, and download additionally requires current access to every represented attachment.
