...
12. During investigation or action-required work, the assigned investigator or a case manager records exactly one immutable conclusion for each server-selected procedure in the approved plan. Each conclusion retains the plan index/text, completed/exception-identified/not-applicable result, summary, optional findings/source reference, complete approved plan/review and exact current case/event snapshots, executor/time, and fingerprint. Newly plan-governed cases cannot resolve until every approved-plan procedure has a retained conclusion.
...
- Procedure results, summaries, findings, not-applicable conclusions, and source references are deliberate authorized-user inputs. Completion proves only that every configured procedure received an attributable in-product conclusion; it does not prove the work occurred, validate sources, infer findings, establish truth or legal sufficiency, or demonstrate investigation quality/effectiveness.
