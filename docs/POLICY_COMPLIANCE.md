...
Approval schedules the first review at the requested frequency, capped by the expiration date. A different `Update Policies` user from both the requester and latest decision maker records an append-only monitoring review with `effective`, `needs_action`, or `revoke_recommended`, a summary, a deliberate control-effectiveness statement, an optional operator-supplied evidence reference, and up to 20 optional accepted audit-evidence files they are authorized to access. The server retains bounded file copies and snapshots their attachment/response/request/audit provenance, metadata, actual size, disk/path, SHA-256, actor, and time. Those manifests are bound into the review fingerprint alongside the approved exception, latest decision, approved policy context, current policy context, actor/time, and next review date. A changed policy context cannot be confirmed effective. `needs_action` and `revoke_recommended` open an owner-attributed issue in the shared governed remediation lifecycle. The `action_required` signal persists through remediation and verification and clears only after independent closure with separately required accepted audit evidence; due and overdue state are derived from the server-owned next-review date.

An approved governed exception remains effective through its configured expiration date. The daily server reconciliation at 00:10 changes it to `expired` on the following day and appends one immutable record containing the prior state, effective expiry time, actual reconciliation time/run identity, complete governed exception/current-policy context, latest monitoring-review snapshot when present, and SHA-256 fingerprint. Reconciliation is idempotent and can also be invoked with `php artisan fynix:reconcile-policy-exceptions`. Open monitoring issues continue to make the exception `action_required` after expiry until independent closure; expiry does not close, resolve, or validate them.

Authorized users inspect complete paginated request, decision, monitoring, expiry-reconciliation, and issue state under **Policies → Policy exception history** or through REST, with private relation-scoped export. Governed file metadata and exact retained-copy downloads appear only when the viewer also passes the current attachment ACL; export omits attachment-derived metadata because queued export does not carry that per-file authority. Existing exception rows without a governance fingerprint are visibly labeled `Legacy` and cannot enter the governed decision, monitoring, or server-expiry service. Previously approved, in-window legacy rows remain eligible for existing attestations for backward compatibility, but are not governed request/decision evidence. The product records deliberate requests, decisions, monitoring judgments, deterministic date reconciliation, remediation handoffs, and independent closure. It does not prove legal approval, quantify risk, make legal expiry judgments beyond enforcing the configured date, verify the monitoring reference, validate compensating-control effectiveness, prove file truth/authenticity/sufficiency, provide electronic signatures, collect evidence automatically, or execute remediation automatically.
...
A policy owner or user with `Update Policies` can launch a campaign from a currently effective, non-retired policy that has readable embedded body content. Launch requires a title, future due time, and one to 500 distinct active users; instructions and a policy comprehension check are optional. REST-authored checks contain one to 20 immutable multiple-choice questions, two to six options each, a 1–100 passing percentage, and one to five allowed attempts. The server allocates a per-policy campaign version and snapshots the policy identity, content, document reference, scope, ownership, dates, revision history, update time, comprehension definition, and separate SHA-256 fingerprints. Later policy edits do not rewrite the assigned version. Launch also writes one in-app database notification per assignment. If any notification is not accepted by that channel, the complete campaign launch rolls back. Each successful delivery retains immutable recipient/campaign snapshots, attempted and delivered timestamps, notification identity, and a SHA-256 fingerprint even if the user later deletes the bell notification.

Assigned employees use **Compliance → My Policy Acknowledgements** to inspect only their assignments, in-app delivery evidence, and the exact policy snapshot. If a check is configured, the workspace exposes questions without answer keys; the assigned employee submits bounded attempts and the server derives the integer percentage from the immutable answer key. Each attempt retains version, selected answers, complete scoring snapshot, result, actor/time, and fingerprint. A passing attempt is required before acknowledgement, and no further attempts are accepted after passing. Acknowledgement requires an explicit accepted confirmation; the server supplies the standard statement and records the assigned user, optional comment/client reference, campaign and policy snapshots, fingerprint, and timestamp. Another user cannot submit an attempt or acknowledge the assignment. A policy editor or owner uses **Compliance → Policy Campaigns** for complete paginated attempt/audience/delivery reporting and scoped export, including the scoring snapshot.

At 08:00 daily, the server evaluates open, unacknowledged assignments for active recipients. It sends one `due_soon` in-app reminder when the due time is within three days and one `overdue` reminder when the due time has passed; each type is idempotent per assignment. A cancelled or failed database-channel insertion rolls back that reminder so a later run can retry. Each successful reminder retains immutable type, recipient/campaign snapshots, notification identity, attempted/delivered times, channel, and SHA-256 fingerprint even if the bell notification is later deleted. Operators may invoke `php artisan fynix:reconcile-policy-acknowledgement-reminders` for recovery.

At 08:10 daily, a separate idempotent process escalates an open, unacknowledged assignment to the current active policy owner only after it has remained overdue for more than three days. The one-per-assignment in-app escalation retains immutable assigned-user, policy-owner, campaign/policy-fingerprint, notification, delivery-time, channel, and SHA-256 evidence. Closed or acknowledged assignments and assignments whose current policy owner is inactive are skipped; failed insertion rolls back for retry. Operators may invoke `php artisan fynix:reconcile-policy-acknowledgement-escalations` for recovery.
...
Audience selection, policy content, questions, answer keys, and passing thresholds remain deliberate operator inputs. Delivery, reminder, and policy-owner escalation evidence prove insertion into Fynix's in-app database-notification store at the recorded time; they do not prove that a human saw or read a notification, and Fynix does not send or prove email/SMS/push delivery. Escalation is to the current policy owner only; Fynix does not synchronize or infer line-management/HR hierarchies. A comprehension score proves only how submitted option indexes compared with the configured answer key; it is not training delivery, learning assurance, identity proofing, or compliance inference. This workflow does not automatically derive audiences, deliver courses, provide qualified electronic signatures, automatically collect evidence, or validate authenticity/sufficiency. Regulatory inventory is deliberate and does not provide feeds or automatic policy-to-law detection. Control-test results must not be inferred from policy attestations or acknowledgements; they are recorded through the separate control-testing workflow.
