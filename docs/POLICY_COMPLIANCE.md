...
Authorized users inspect complete paginated request, decision, monitoring, and issue state under **Policies → Policy exception history** or through REST, with private relation-scoped export. Existing exception rows without a governance fingerprint are visibly labeled `Legacy` and cannot enter the governed decision or monitoring service. Previously approved, in-window legacy rows remain eligible for existing attestations for backward compatibility, but are not governed request/decision evidence. The product records deliberate requests, decisions, monitoring judgments, remediation handoffs, and independent closure. It does not prove legal approval, quantify risk, verify the monitoring reference, validate compensating-control effectiveness, provide electronic signatures, make automatic expiry decisions, collect evidence automatically, or execute remediation automatically.
