...
return new class extends Migration
{
    ...
    {
        if (Schema::hasTable('ai_monitoring_review_evidence')) {
            return;
        }

        Schema::create('ai_monitoring_review_evidence', function (Blueprint $table): void {
            $table->id();
            $table->foreignId('ai_monitoring_review_id')->constrained()->restrictOnDelete();
            ...
            $table->timestamps();
            $table->unique(['ai_monitoring_review_id', 'file_attachment_id'], 'ai_monitoring_review_evidence_unique');
        });
    }

    public function down(): void
    {
        // Governed AI monitoring evidence is retained during routine code rollback.
    }
};
