...
class UpdateRiskIndicatorRequest extends FormRequest
{
    ...
    {
        /** @var RiskIndicator $indicator */
        $indicator = $this->route('indicator');

        return [
            'owner_id' => ['required', 'integer', 'exists:users,id'],
            'code' => ['required', 'string', 'max:100', Rule::unique('risk_indicators')->where('risk_id', $indicator->risk_id)->ignore($indicator->id)],
