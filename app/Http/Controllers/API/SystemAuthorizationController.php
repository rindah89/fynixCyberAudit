...
class SystemAuthorizationController extends Controller
{
    ...
    {
        return response()->json(SystemAuthorizationPackage::query()->with(['application:id,name,owner_id', 'submitter:id,name', 'latestDecision.authorizer:id,name'])->latest('id')->paginate($request->integer('per_page', 50)));
    }
    ...
    {
        return response()->json(['data' => $package->load(['application.owner:id,name,email', 'submitter:id,name', 'decisions.authorizer:id,name'])]);
    }
