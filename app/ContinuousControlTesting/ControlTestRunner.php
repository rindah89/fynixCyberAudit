...
class ControlTestRunner
{
    ...
    public function execute(
        ControlTestDefinition $definition,
        User $actor,
        ...
        try {
            return DB::transaction(function () use ($definition, $actor, $observedValue, $notes, $evidenceReference, $evidenceAttachmentIds, $executedAt, $snapshotter, $snapshotBatch, &$retainedCopies) {
                $locked = ControlTestDefinition::query()->lockForUpdate()->findOrFail($definition->id);
                $control = Control::query()->lockForUpdate()->findOrFail($locked->control_id);
                $locked->setRelation('control', $control);
                if (! $actor->can('execute', $locked)) {
                    abort(403, 'You cannot record an observation for this control test.');
                }
                if (! $locked->is_active) {
                ...
                foreach ($evidenceSnapshots as $snapshot) {
                    $execution->evidence()->create($snapshot + [
                        'linked_by' => $actor->id,
                        'linked_at' => $executedAt,
                    ]);
                }
                ...
                return $execution->load(['finding', 'evidence.linkedBy:id,name']);
            }, 3);
