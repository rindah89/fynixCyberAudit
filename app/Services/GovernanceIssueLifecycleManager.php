...
class GovernanceIssueLifecycleManager
{
    ...
    public function close(Model $issue, User $actor, array $data): GovernanceIssueLifecycle
    {
        ...
        return DB::transaction(function () use ($issue, $actor, $data): GovernanceIssueLifecycle {
            ...
            $this->recordTransition($lifecycle, $from, GovernanceIssueStatus::Closed, $actor, 'Independent closure verification completed.', $task, $data['verification_summary'], $data['evidence_reference'] ?? null);
    ...
    private function recordTransition(GovernanceIssueLifecycle $lifecycle, ?GovernanceIssueStatus $from, GovernanceIssueStatus $to, User $actor, string $rationale, ?RemediationTask $task = null, ?string $verificationSummary = null, ?string $evidenceReference = null): void
    {
        $lifecycle->transitions()->create([
    ...
    private function load(GovernanceIssueLifecycle $lifecycle, Model $issue): GovernanceIssueLifecycle
    {
        ...
        return $lifecycle->load(['remediationTask', 'verifier:id,name', 'closer:id,name', 'transitions.actor:id,name']);
    }
}
