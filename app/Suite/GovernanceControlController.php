...
class GovernanceControlController
{
    public function store(Request $request, DataGovernanceControlService $service): JsonResponse
    {
        ...
        $validator = Validator::make(is_array($body) ? $body : [], [
            ...
            'command' => ['required', 'in:privacy_request.open,privacy_request.close,retention_policy.define,retention.disposition.record,legal_hold.place,legal_hold.release,processor.register,recovery_evidence.record'],
        ...
        $payloadValidator = Validator::make($body['payload'], match ($body['command']) {
            ...
            'legal_hold.place' => ['retention_policy_id' => ['required', 'integer', 'min:1'], 'reason' => ['required', 'string', 'max:1000'], 'record_ref' => ['nullable', 'regex:/^[A-Za-z0-9][A-Za-z0-9._:-]{0,254}$/'], 'source_hold_ref' => ['nullable', 'regex:/^[A-Za-z0-9][A-Za-z0-9._:-]{0,254}$/']],
