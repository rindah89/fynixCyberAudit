...
class MessagesRelationManager extends RelationManager
{
    ...
    {
        return $table->modifyQueryUsing(fn ($query) => $query->with('actor:id,name,email'))
            ->columns([
                ...
                Action::make('inspect')->modalSubmitAction(false)->modalCancelActionLabel(__('Close'))
                    ->schema(fn (Schema $schema): Schema => $schema->components([
                        Text::make(fn ($record): string => json_encode($record->attributesToArray(), JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE)),
                    ])),
            ])->defaultSort('version');
    }
}
