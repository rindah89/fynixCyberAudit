...
class PolicyExceptionMonitoringManager
{
    public function history(PolicyException $exception, User $actor): Builder
    {
        ...
        return $exception->monitoringReviews()->with([
            'reviewer:id,name', 'issue.lifecycle', 'evidence.linkedBy:id,name',
            'evidence.attachment.audit.members',
            'evidence.attachment.dataRequestResponse.dataRequest.audit.members',
        ])->latest('version')->getQuery();
    }

    public function visibleReview(PolicyExceptionMonitoringReview $review, User $actor): PolicyExceptionMonitoringReview
    {
        $visible = clone $review;
        $visible->setRelation('evidence', $this->authorizedEvidence($review, $actor)->map(function (PolicyExceptionMonitoringReviewEvidence $evidence): PolicyExceptionMonitoringReviewEvidence {
            $copy = clone $evidence;
            $copy->unsetRelation('attachment');
    ...
    /** @return Collection<int, PolicyExceptionMonitoringReviewEvidence> */
    private function authorizedEvidence(PolicyExceptionMonitoringReview $review, User $actor): Collection
    {
        return $review->evidence->filter(fn (PolicyExceptionMonitoringReviewEvidence $evidence): bool => $evidence->attachment !== null
    ...
    public function review(PolicyException $exception, User $actor, array $data): PolicyExceptionMonitoringReview
    {
        ...
        try {
            return DB::transaction(function () use ($exception, $actor, $data, $snapshotter, $snapshotBatch, &$retainedCopies): PolicyExceptionMonitoringReview {
                ...
                $evidenceSnapshots = empty($validated['evidence_attachment_ids']) ? [] : $snapshotter->snapshot(
                    $validated['evidence_attachment_ids'],
                    $actor,
                    'policy-exception-monitoring-review',
                    $snapshotBatch,
                    $retainedCopies,
                );
                $evidenceManifest = collect($evidenceSnapshots)->map(fn (array $evidence): array => $evidence + [
                    'linked_by' => $actor->id,
                    'linked_at' => $reviewedAt->toISOString(),
                ])->all();
                $payload = [
                ...
                $review = $locked->monitoringReviews()->create(collect($payload)->except('evidence_manifest')->all() + [
                    'fingerprint' => hash('sha256', json_encode($payload, JSON_THROW_ON_ERROR | JSON_UNESCAPED_SLASHES)),
                ]);
                foreach ($evidenceManifest as $evidence) {
                    $review->evidence()->create($evidence);
                }
                $locked->update([
                ...
                return $review->load([
                    'reviewer:id,name', 'exception.policy:id,code,name', 'issue.lifecycle', 'evidence.linkedBy:id,name',
                ]);
            }, 3);
    ...
    public static function rules(): array
    {
        return [
            ...
            'evidence_attachment_ids' => ['sometimes', 'array', 'min:1', 'max:20'],
