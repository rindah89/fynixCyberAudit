        ...
        <div class="rounded-lg border p-3">
            ...
            @if ($request->closure)
                <div class="mt-2 rounded-lg border p-2">
                    ...
                    @endif
                    @if ($request->closure->acknowledgement)
                        <div>Closure acknowledged by {{ $request->closure->acknowledgement->recipient?->name ?? data_get($request->closure->acknowledgement->recipient_snapshot, 'name') }} · {{ $request->closure->acknowledgement->acknowledged_at?->toDayDateTimeString() }}</div>
                        <details class="mt-1"><summary class="font-medium">Retained closure-acknowledgement evidence</summary><pre class="mt-1 overflow-auto whitespace-pre-wrap text-xs">{{ json_encode(['recipient' => $request->closure->acknowledgement->recipient_snapshot, 'closure' => $request->closure->acknowledgement->closure_snapshot, 'delivery' => $request->closure->acknowledgement->delivery_snapshot], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) }}</pre></details>
                        <div class="break-all font-mono text-xs">Acknowledgement fingerprint: {{ $request->closure->acknowledgement->fingerprint }}</div>
                        @foreach ($request->closure->acknowledgement->internalDeliveries as $internalDelivery)
