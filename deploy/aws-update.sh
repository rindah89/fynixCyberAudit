...
[[ "$previous_revision" =~ ^[0-9a-f]{40}$ && "$previous_image" =~ ^sha256:[0-9a-f]{64}$ ]] || { echo "verified previous release tuple is required before deployment" >&2; exit 1; }
if [[ "$previous_revision" =~ ^[0-9a-f]{40}$ && "$previous_image" =~ ^sha256:[0-9a-f]{64}$ ]]; then
  ...
  if [[ ! -d "$release_root/$previous_revision" ]]; then mkdir "$release_root/$previous_revision"; rsync -a --exclude=.env --exclude=.git-revision --exclude=.image-id "$target/" "$release_root/$previous_revision/"; fi
fi
rollback(){ rc=$?; trap - ERR; if [[ $mutated == 1 && "$previous_revision" =~ ^[0-9a-f]{40}$ && -d "$release_root/$previous_revision" && "$previous_image" =~ ^sha256:[0-9a-f]{64}$ ]]; then rsync -a --delete --exclude=.env --exclude=.git-revision --exclude=.image-id "$release_root/$previous_revision/" "$target/"; CYBERAUDIT_IMAGE="fynix-cyberaudit-release:$previous_revision" FYNIX_RELEASE_SHA="$previous_revision" docker compose -p fynixcyberaudit --env-file "$target/.env" -f "$target/docker-compose.yml" up -d --no-build; prior_container="$(docker compose -p fynixcyberaudit --env-file "$target/.env" -f "$target/docker-compose.yml" ps -q app)"; for _ in $(seq 1 60); do [[ "$(docker inspect -f '{{.State.Health.Status}}' "$prior_container" 2>/dev/null || true)" == healthy ]]&&break; sleep 2; done; test "$(docker inspect -f '{{.Image}}' "$prior_container")" = "$previous_image"; test "$(docker inspect -f '{{.State.Health.Status}}' "$prior_container")" = healthy; fi; exit $rc; }; trap rollback ERR
