...
payload="$(aws secretsmanager get-secret-value --region "$region" --secret-id "$secret_id" --query SecretString --output text)"
PAYLOAD="$payload" python3 - "$stage" <<'PY'
import base64,json,os,pathlib,sys
p=json.loads(os.environ['PAYLOAD']); expected={'requester_key','signing_private_key','signing_key_id','signing_public_keys','executive_public_keys'}
...
root=pathlib.Path(sys.argv[1]); files={'change-requester-key':p['requester_key'],'change-signing-private-key':p['signing_private_key'],'change-signing-public-keys.json':p['signing_public_keys'],'executive-public-keys.json':p['executive_public_keys']}
...
find "$stage" -maxdepth 1 -type f -exec install -m 0600 {} "$destination"/ \;
